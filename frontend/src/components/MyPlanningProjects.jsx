...
function ProjectCard({ project, onDelete, onEdit, onOpen }) {
  ...
  return (
    <article className="planning-project-card">
      ...
      <dl>
        <div><dt><MapPinned size={14} />Site area</dt><dd>{project.area_acres ? `${Number(project.area_acres).toLocaleString()} acres` : 'Boundary pending'}</dd></div>
...
export function MyPlanningProjects({ cities, projects, loading, error, onDelete, onLogout, onOpen, onSave, user }) {
  ...
  return (
    <main className="project-library">
      <header className="project-library-topbar"><div className="brand"><span className="brand-mark"><Building2 size={20} /></span><div><strong>CITYMIND</strong><small>Development Feasibility</small></div></div><div className="project-account"><span><strong>{user?.name ?? 'CityMind User'}</strong><small>{user?.email ?? 'Private workspace'}</small></span><span className="avatar">{(user?.name ?? 'CM').split(' ').map((part) => part[0]).slice(0, 2).join('')}</span>{onLogout && <button onClick={onLogout} type="button">Sign out</button>}</div></header>
      <section className="project-library-heading"><div><p className="eyebrow">Private development portfolio</p><h1>My Development Projects</h1><p>Create or open a site to assess development readiness, infrastructure gaps, supported risks, and next investigation priorities.</p></div><button className="future-create-button" onClick={() => setEditor(null)} type="button"><Plus size={15} />Assess New Site</button></section>
      ...
      {!loading && !error && projects.length === 0 && <div className="project-library-state"><MapPinned size={25} /><p>No development projects yet. Add the first site to assess.</p></div>}
