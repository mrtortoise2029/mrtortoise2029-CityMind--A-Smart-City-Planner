...
import { AlertTriangle, CalendarClock, Database, MapPin, Users } from 'lucide-react';
...
export function GrowthPredictionView({ onOpenMap, project }) {
  ...
  const chartData = data.current?.population ? [data.current, ...data.scenarios] : data.scenarios;
  return (
    <article className="growth-prediction-view">
      <header><div><p className="eyebrow">Deterministic scenario planning</p><h2>Growth Prediction</h2><p>Population-led demand within the saved project planning horizon.</p></div><div className="analysis-header-actions"><span className="simulation-label">{data.projection_label}</span>{onOpenMap && <button onClick={onOpenMap} type="button"><MapPin size={13} />View demand on GIS</button>}</div></header>
      ...
      <section className="growth-chart-card"><header><div><h3>Population scenario</h3><p>Current evidence is shown at year 0 when available. Values beyond the project horizon are not shown.</p></div></header><div className="growth-chart"><ResponsiveContainer height="100%" width="100%"><LineChart data={chartData}><CartesianGrid stroke="#203a32" strokeDasharray="3 3" /><XAxis dataKey="year" stroke="#789088" tickFormatter={(year) => `${year}y`} /><YAxis stroke="#789088" width={65} /><Tooltip contentStyle={{ background: '#10211d', border: '1px solid #34594b' }} formatter={(value) => number(value)} /><Line dataKey="population" dot={{ fill: '#63dbaa' }} stroke="#63dbaa" strokeWidth={3} type="monotone" /></LineChart></ResponsiveContainer></div></section>
      ...
      <div className="analysis-notes"><section><h3>Assumptions</h3>{data.assumptions.map((item) => <p key={item}>{item}</p>)}</section><section><h3>Data provenance</h3>{data.data_sources.map((item) => <p key={item.dataset}><b>{item.dataset}</b><span>{item.source ?? 'Data unavailable'} · {item.data_type.replaceAll('_', ' ')}{item.data_year ? ` · ${item.data_year}` : ''}</span></p>)}</section></div>
